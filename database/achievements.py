...
def check_trackers(player: models.Player, results: List[models.GameResult]) -> models.Player:
    """
        Обновление трекеров игрока, не зависящих от других игроков
    """

    trackers = {"1": 0, "1_2_3": 0, "2_3": 0, "4": 0, "other": 0, "lost": 0, "wrong": 0}
    for result in results:
        ...
        if result.error_code != Error.default_value:
            continue

        if result.position == 0:
            trackers["1"] += 1
        if result.position in (0, 1, 2):
            trackers["1_2_3"] += 1
        if result.position in (1, 2):
            trackers["2_3"] += 1
    ...
    update_tracker(player, Achievement.strategy_lost, trackers["lost"])
    update_tracker(player, Achievement.wrong_res, trackers["wrong"])
    update_tracker(player, Achievement.habitue, trackers["other"])
    update_tracker(player, Achievement.almost_there,
                   1 if trackers["4"] > 0 else 0)
    update_tracker(player, Achievement.the_night_is_still_young,
                   1 if trackers["2_3"] > 0 else 0)
    update_tracker(player, Achievement.automerge_king,
                   trackers["1_2_3"])
    update_tracker(player, Achievement.first_among_equals,
                   1 if trackers["1"] > 0 else 0)

    return player


def update_players_trackers(users: List[PlayerInfo]) -> None:
    """
    ...
    players = list(models.Player.objects(gitlab_id__in=[u.gitlab_id for u in users]))

    results = list(models.GameResult.objects(player__in=players))

    for player in players:
        check_trackers(
            player,
            [i for i in results if i.player == player],
        ).save()
...
def update_players_results(game_name: str, results: List[PlayerResult]) -> None:
    ...
    players = list(models.Player.objects(
    ...
    found_res = list(models.GameResult.objects(game=game, player__in=players))
    new_res = [
    ...
    update_players_trackers([PlayerInfo(i.name, i.gitlab_id) for i in results])


def get_players_achievements(players_ids: List[str]) -> List[PlayerAchievements]:
    """
